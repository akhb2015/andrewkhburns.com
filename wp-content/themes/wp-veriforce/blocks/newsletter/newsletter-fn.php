<?php

add_action('acf/init', function() {

	// REGISTER BLOCK TYPE

	if (function_exists('acf_register_block_type')) {
		acf_register_block_type([
			'title' => 'Newsletter',
			'name' => 'newsletter',
			'supports' => ['align' => false],
			'render_template' => 'blocks/newsletter/newsletter.php',
			'enqueue_assets' => function() {
				wp_enqueue_style('block-newsletter', get_template_directory_uri().'/blocks/newsletter/newsletter.css', null, null);
			}
		]);
	}



	// ADD ACF FIELDS

	acf_add_local_field_group([
		'key' => 'group_newsletter',
		'title' => 'Block: Newsletter',
		'fields' => [[
			'key' => 'field_newsletter_display',
			'label' => 'Display',
			'name' => 'display',
			'type' => 'accordion',
			'open' => 1,
			'multi_expand' => 1,
		], [
			'key' => 'field_newsletter_content',
			'label' => 'Content',
			'name' => 'content',
			'type' => 'wysiwyg',
			'wrapper' => ['width' => '80', 'class' => 'td-short'],
		], [
			'key' => 'field_newsletter_image',
			'label' => 'Image',
			'name' => 'image',
			'type' => 'image',
			'return_format' => 'id',
			'wrapper' => ['width' => '20'],
		], [
			'key' => 'field_newsletter_options',
			'label' => 'Options',
			'name' => 'options',
			'type' => 'accordion',
			'multi_expand' => 1,
		], [
			'key' => 'field_newsletter_style',
			'label' => 'Style',
			'name' => 'style',
			'type' => 'select',
			'choices' => [
				'placeholder' => 'Placeholder',
			],
			'allow_null' => 1,
			'wpml_cf_preferences' => 0,
		]],
		'location' => [[[
			'param' => 'block',
			'operator' => '==',
			'value' => 'acf/newsletter',
		]]],
		'acfml_field_group_mode' => 'advanced',
	]);
});

...
const DOWNLOADS = gql`
  query Downloads($page: Float, $pageSize: Float) {
    downloads(page: $page, pageSize: $pageSize) {
      query
      currentPage
      perPage
      totalPages
...
export const Downloads: React.FC = () => {
  ...
  const {
    isFetching,
    isLoading,
    refetch: getDownloadResults,
  } = useQuery(
    'DOWNLOADS',
    async () =>
      graphQLClient.request(DOWNLOADS, {
        page,
        pageSize: PAGE_SIZE,
      }),
    {
      enabled: false,
      onSuccess: ({ downloads: { results, totalPages: _totalPages } }: unknown) => {
        setList(() => results);
